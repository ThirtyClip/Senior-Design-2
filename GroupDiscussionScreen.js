...
<View style={{ padding: 20 }}>
			<Text style={{ fontSize: 20, fontWeight: 'bold', marginBottom: 10 }}>Discussion Forum</Text>

            <FlatList
                data={discussions}
                keyExtractor={(item) => item.id.toString()}
                renderItem={({ item }) => (
                    <TouchableOpacity onPress={() => handleForumPress(item.id, item.title)}>
                        <View style={styles.forumItem}>
                            <Text style={styles.forumTitle}>{item.title}</Text>
                            <Text style={styles.forumInfo}>Posted by {item.author} - {item.timestamp?.toDate().toLocaleString()}</Text>
